...
class Login extends Controller
{
    public static function main_()
    {
        if (isset($_POST['login']))
        {
            ...
            {
                if(password_verify($password, static::query('SELECT password FROM camagru.users WHERE username=:username', array(':username'=>$username))[0]['password']))
                {
                    $bother = True;
                    ...
                    $token = sha1($toke);
                    static::query('INSERT INTO camagru.tokens (token, user_id) VALUES (:token, :user_id)', array(':token'=>$token, ':user_id'=>$user_id));

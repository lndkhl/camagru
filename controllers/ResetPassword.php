...
class ResetPassword extends Controller
{
    public static function main_()
    {
        if (isset($_POST["resetpassword"]))
        {
            ...
            {
                $user_id = static::query('SELECT id FROM camagru.users WHERE email=:email', array(':email'=>$email))[0]['id'];
                $token = sha1($toke);
                static::query('INSERT INTO password_tokens (token, user_id) VALUES (:token, :user_id)', array(':token'=>$token, ':user_id'=>$user_id));

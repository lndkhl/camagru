...
class Login
{
    public static function isLoggedIn()
    {
        if(isset($_COOKIE["CID"]))
        {
            ...
            {
                $idnum = DB::query('SELECT user_id FROM tokens WHERE token= :token', array(':token'=>$hashcookie))[0]['user_id'];
                $uid = DB::query('SELECT username FROM users WHERE id= :user_id', array(':user_id'=>$idnum))[0]['username'];
                ...
                else
                {
                    ...
                    $token = sha1($toke);
                    DB::query('INSERT INTO tokens (token, user_id) VALUES (:token, :user_id)', array(':token'=>$token, ':user_id'=>$idnum));
